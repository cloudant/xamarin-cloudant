...
namespace Com.Cloudant.Client
{
	...
	public class Database
	{
		...
		public Database(CloudantClient client, String name) {
			this.client = client;
			dbname = name;
			remoteUri = new Uri(this.client.accountURL + WebUtility.UrlEncode(name));
		}
		...
		public Task<DocumentRevision> find(String documentId){
			...
			try {
				...
				Uri requestUrl = new Uri (this.remoteUri.ToString() + "/" + documentId);
				Debug.WriteLine ("fetch document remote URL is: "+requestUrl.ToString());
				...
				Task<DocumentRevision> result = Task.Run (() => {
					...
					Task<DocumentRevision> revisionTask = httpTask.ContinueWith( (antecedent) => {
						...
						Debug.WriteLine("Cloudant find document response.  Remote URL: " + requestUrl.ToString() + ". " +
		...
		public Task<String> remove(DocumentRevision revisionToRemove){
			...
			try {
				...
				Uri requestUrl = new Uri(this.remoteUri.ToString() + "/" 
					+ revisionToRemove.docId + "?rev=" + revisionToRemove.revId);
				Task<String> result = Task.Run (() => {
					...
					Task<String> responseTask = httpTask.ContinueWith( (antecedent) => {
						...
						Debug.WriteLine("Cloudant delete document response.  Remote URL: " + requestUrl.ToString() + ". " +
		...
		public Task createIndex(String indexDefinition) {
			...
			Task result = Task.Run (() => {
				...
				Uri indexUri = new Uri (remoteUri.ToString () + "/_index");
				Debug.WriteLine ("indexUri: " + indexUri);
		...
		public Task<List<DocumentRevision>> findByIndex(String selectorJson, FindByIndexOptions
			...
			Task<List<DocumentRevision>> result = Task.Run <List<DocumentRevision>> ( () => {
				Uri indexUri = new Uri(remoteUri.ToString()+"/_find");
		...
		public Task deleteIndex(String indexName, String designDocId) {
			...
			Task result = Task.Run( () => {
				Uri indexUri = new Uri(remoteUri.ToString()+"/_index/"+designDocId+"/json/"+indexName);
		...
		private Task<DocumentRevision> createDocumentRevision(DocumentRevision doc){
			...
			try {
				...
				if (doc.docId != null && doc.revId != null){
					uri = new Uri (this.remoteUri.ToString()+"/"+doc.docId+"?rev=" +doc.revId);
				} else {
					uri = this.remoteUri;
				}
				Debug.WriteLine ("remote URL is: "+uri.ToString());
		...
		private Task<DocumentRevision> updateDocumentRevision(DocumentRevision doc){
			...
			try {
				Uri uri = null;
				if (doc.docId != null){
					uri = new Uri (this.remoteUri.ToString()+"/"+doc.docId);
				} else {
